  ...
  ['block'](event, blockInfo) {
    if(blockInfo != null) {
      ...
      if(!db){
        ...
        const errThree = dbInit.createSendTable("record", db);
        if(errThree === "errtwo"){
      ...
      var req = https.request(options, function (res) {
        ...
        res.on('data', function (data) {
          ...
          if(!db){
            ...
            const sql = "SELECT send_id, account_id, send_time, account_addr_from, account_addr_to, send_balance, service_charge, comment, send_status, block_number " +
            ...
            db.all(sql, function w(err, row) {
              ...
              for(let i = 0; i < row.length; i++) {
                if(blockNumber - parseInt(row[i].block_number) > 3) {
                  const modify = db.prepare("UPDATE record set send_status=?, block_number=? where send_id = ?");
