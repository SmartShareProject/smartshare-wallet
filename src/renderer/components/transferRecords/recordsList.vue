      ...
      <div class="record_head">
        <ul v-for="(item,index) in accountList" v-show="index == accountShowNo">
          ...
          <li>
            <span>账户余额：</span>
            <div class="right_div">
              <p class="orange_p">{{accountBalance}} SPP</p>
            </div>
          </li>
        </ul>
      </div>
      <div class="record_table">
        ...
        <ul>
          <li v-for="elem in recordsList">
            <div>
              <p>{{isTransferIn?elem.sendTime:elem.send_time}}</p>
            </div>
            <div>
              <p>{{isTransferIn?elem.sendBalance:elem.send_balance}}</p>
            </div>
            <div>
              <p>{{isTransferIn?elem.toAddress:elem.account_addr_to}}</p>
            </div>
            <div>
              <p>{{isTransferIn?(elem.sendState!=0?"转入":"转出"):(elem.send_status!=0?"转入":"转出")}}</p>
            </div>
            <div>
              <p>{{isTransferIn?(getStatusName(elem.status)):(getStatusName(elem.send_status))}}</p>
            </div>
            <div>
              <p>{{isTransferIn?elem.sendFee:elem.service_charge}}SSP</p>
            </div>
            <div>
              <p>{{isTransferIn?elem.sendHash:elem.trans_hash}}</p>
            </div>
            <div>
              <p>{{isTransferIn?elem.sendComment:elem.comment}}</p>
            </div>
          </li>
        </ul>
      </div>
...
//        alert(item.account_address)
        let _this = this;
        let params = {
          toAddress:'LET'+item.account_address
        }
